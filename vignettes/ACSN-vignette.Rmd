...
names(ACSNEnrichment::ACSN_maps)

````
`````{r gmt_map_show, echo = FALSE}
knitr::kable(names(ACSNEnrichment::ACSN_maps))
````

````{r gmt_access_code, eval = FALSE}
#And accessing them:
ACSNEnrichment::ACSN_maps$CellCycle
````
...
````{r heatmap_plot}
ACSNEnrichment::represent_enrichment(enrichment = list(
    SampleA = ACSNEnrichment::enrichment_test[1:10,], 
