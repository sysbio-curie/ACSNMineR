...
recently released Atlas of Cancer Signaling Networks (ACSN). All these open and
...
encoded as a GMT file, such as any of the gene signatures available in the
...
\Keywords{biological module, enrichment, depletion, cancer, maps, molecular networks, molecular pathways, \proglang{R}}
\Plainkeywords{biological module, enrichment, depletion, cancer, maps, molecular networks, molecular pathways, R} %% without formatting
...
\code{enrichment}, offers many options to set the statistical test used (either
...
(\code{represent\_enrichment}) or to perform calculations for multiple samples
or groups (\code{multisample\_enrichment}). Large-scale molecular maps of the
...
\code{format\_from\_gmt}. 

We also describe in this work the \pkg{RNaviCell} package, a R package highly
complementary to \pkg{ACSNMineR}. This package is dedicated to create data
...
Of course, \pkg{ACSNMineR} is not the only R package for enrichment calculations. For
instance, \pkg{GOstats} \citep{falcon2007using} is probably one of the first
packages that was created to calculate enirchment for Gene Ontology categories.
\pkg{GOstats} can also be used to calculate enrichment for other biological
pathways categories, such as KEGG pathways (by using an instance of the class
\code{KEGGHyperGParams}) or PFAM protein families (using
\code{PFAMHyperGParams}). However, its usage might not be as straightforward as
\pkg{ACSNMineR}, and it does not seems possible to test user-defined biological
...
versions of the \pkg{ACSNMineR} package and Eric Viara for guidance and
assistance on the development of the \pkg{RNaviCell} package.
