...
genes differentially expressed on the ACSN master map using \code{ACSNMineR}
...
information \citep{hawkins2010next}. Integrating these
heterogeneous datasets provides more biological insights than performing
separate analyses. 

For instance, international consortia such as The Cancer Genome Atlas (TCGA)
have launched large-scale
initiatives to characterize multiple types of cancer at different levels
(genomic, transcriptomic, epigenomic, etc.) on several hundreds of samples.
These integrative studies have already led to the identification of novel cancer
genes \citep{mclendon2008comprehensive}.

Glioblastoma multiforme (GBM) is the most malignant form of gliomas, and despite
intense investigation of this disease in the past decades, most patients with
GBM die within approximately 15 months of diagnosis \citep{maher2001malignant}. 
...
mat <- n$readDatatable('gbm.txt')
n$importDatatable("Mutation data", "GBM", mat)

n$drawingConfigSelectGlyph(1, TRUE)

n$glyphEditorOpen(1)
n$glyphEditorSelectSample(1, "categ")
n$glyphEditorSelectShapeDatatable(1, "GBM")
n$glyphEditorSelectColorDatatable(1, "GBM")
n$glyphEditorSelectSizeDatatable(1, "GBM")
n$glyphEditorApply(1)

n$unorderedConfigSetDiscreteShape("GBM", "sample", 0, 1)
n$unorderedConfigSetDiscreteShape("GBM", "sample", 1, 5)
n$unorderedConfigApply("GBM", "shape")

n$unorderedConfigSetDiscreteColor("GBM", "sample", 0, "398BC3")
n$unorderedConfigSetDiscreteColor("GBM", "sample", 1, "CC5746")
n$unorderedConfigApply("GBM", "color")

n$unorderedConfigSetDiscreteSize("GBM", "sample", 0, 4)
n$unorderedConfigSetDiscreteSize("GBM", "sample", 1, 14)
n$unorderedConfigApply("GBM", "size")
